...
import { Map, Users, MoreVertical, Pencil, Trash2 } from "lucide-react";
    ...
    >
      <div className="flex flex-col gap-3 p-5">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${STAGE_STYLES[project.stage]}`}>
            {project.stage}
          </span>
        ...
        <div>
          <div className="mb-1.5 flex items-center justify-between text-xs">
            ...
            <span className="font-medium text-foreground">
              {project.roadmapProgress}% · {project.roadmapStepsCompleted}/{project.roadmapStepsTotal} étapes
            </span>
          </div>
      ...
      <div className="mt-auto flex flex-wrap items-center gap-2 border-t border-border/60 bg-muted/20 px-5 py-3">
        ...
        {project.needs?.map((need) => (
          <button
            key={need}
            type="button"
            onClick={() => onViewNeed?.(project.id, need)}
