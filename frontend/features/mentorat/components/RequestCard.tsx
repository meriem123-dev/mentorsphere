...
import { Eye, Quote, UserCheck, UserX, ExternalLink } from "lucide-react";
...
export function RequestCard({
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-1 gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
              ...
              <span
                className={`rounded-full border px-2 py-0.5 text-xs font-medium ${STAGE_STYLES[request.stage]}`}
              >
                {request.stage}
              </span>
              <span className="text-xs text-muted-foreground">
                Il y a {request.timeAgo}
              </span>
            </div>

            <div className="relative mt-3 rounded-xl bg-muted/50 py-2.5 pl-8 pr-3">
              <Quote
                className="absolute left-2.5 top-2 h-4 w-4 text-brand-blue/30"
                aria-hidden
              />
              <p className="text-sm italic text-muted-foreground wrap-break-word">
                {request.message}
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-border/60 bg-muted/20 px-5 py-3 pl-6">
        <button
          type="button"
        ...
        {onViewProfile && (
          <Button
            variant={"outline"}
            onClick={() => onViewProfile(request.entrepreneurId)}
            className="ml-auto inline-flex items-center justify-center gap-1.5 rounded-full border border-border px-4 py-1 text-sm font-medium text-brand-blue transition-colors hover:bg-brand-blue/10"
          >
