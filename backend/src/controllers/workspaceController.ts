...
import { getWorkspaceMessages } from "../services/workspaceService";
...
export async function getWorkspaceOverviewHandler(req: Request, res: Response) {
  try {
    ...
    if (result === "FORBIDDEN") {
      return res
        .status(403)
        .json({ message: "Accès non autorisé à ce workspace" });
    }
    ...
    const message =
      err instanceof Error
        ? err.message
        : "Erreur lors du chargement du workspace";
    return res.status(500).json({ message });
  }
}

//recup workspaces du user
export async function getWorkspaceSummariesHandler(
  req: Request,
  res: Response,
) {
  try {
...
//recup historique des messages
export async function getWorkspaceMessagesHandler(req: Request, res: Response) {
  try {
    const id = req.params.id as string;
    const userId = req.user!.userId;

    const result = await getWorkspaceMessages(id, userId);
